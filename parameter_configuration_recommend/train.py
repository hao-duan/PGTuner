...
if __name__ == '__main__':
    ...
    predict_model_save_path = os.path.join(parent_directory, 'query_performance_predict/model_checkpoints/{}_{}_{}_{}_checkpoint.pth'.format(
    ...
    feature_df = def_df[['SIZE', 'q_SIZE', 'DIM', 'LID', 'Sum_K_MinDist', 'Sum_K_MaxDist', 'Sum_K_StdDist', 'q_K_MinRatio',
    ...
    env = IndexEnv(num_dataset, final_default_performance, target_rec_lis, args_r, args_p, predict_model_save_path, standard_path, device)
        ...
        if (episode + 1) % 100 == 0:
            logger.info(
                "[Episode: {}][Average] step: {}s env step: {}s add step: {}s train step: {}s action time: {}s".format(
                    episode, np.mean(step_times), np.mean(env_step_times), np.mean(add_step_times),
                    np.mean(train_step_times), np.mean(action_step_times)))
